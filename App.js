...
export default function App() {
  ...
  const renderPipeline = () => <Section title="Repurpose Pipeline"><Card title="Add Asset"><TextInput value={newAsset} onChangeText={setNewAsset} placeholder="Audio, video, idea, email, offer..." placeholderTextColor={COLORS.muted} style={styles.input} /><PrimaryButton label="+ Add to Pipeline" onPress={addAsset} /></Card><View style={styles.stageRail}>{stages.map(s => <Text key={s} style={styles.stageChip}>{s}</Text>)}</View>{pipeline.map(item => <Card key={item.id} title={item.asset} right={<Pill tone={item.stage === 'Offer' ? 'green' : item.stage === 'Post' ? 'yellow' : 'teal'}>{item.stage}</Pill>}><Text style={styles.body}>Owner: {item.owner}</Text><Text style={styles.muted}>Value: {item.value}</Text><GhostButton label="Move Forward" onPress={() => moveAsset(item.id)} /></Card>)}</Section>;
